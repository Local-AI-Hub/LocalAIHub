...
export default function App() {
  ...
  async function loadOllamaModels(options = {}) {
    ...
    const result = await window.nestAI.listOllamaModels();

    if (!result?.ok) {
      const message = result?.message || 'NestAI could not load your local Ollama models.';
    ...
    setOllamaSelectedModel((current) => (current && models.some((model) => model.name === current) ? current : models[0]?.name || ''));
    setOllamaNotice(
      models.length
  ...
  async function sendOllamaMessage() {
    ...
    const result = await window.nestAI.chatWithOllama({
      model: ollamaSelectedModel,
      messages: nextMessages,
    });

    if (!result?.ok) {
      const message = result?.message || 'NestAI could not get a reply from Ollama.';
      pushToast(message, 'error');
    ...
    setOllamaMessages((current) => [...current, result.data?.message || { role: 'assistant', content: 'Ollama returned an empty reply.' }]);
  ...
  function applyStateResponse(result) {
    if (!result?.ok) {
      throw new Error(result?.message || 'NestAI could not complete that action.');
    }
  ...
  async function loadState() {
    const result = await window.nestAI.bootstrap();
    if (!result?.ok) {
      pushToast(result?.message || 'NestAI could not load the dashboard.', 'error');
      setIsLoading(false);
      return;
    }
  ...
  async function runAction(key, operation) {
    ...
    try {
      ...
      const baseMessage = error.message || 'NestAI could not complete that action.';
      const helpMessage =
  ...
  useEffect(() => {
    loadState();
    const unsubscribeInstallProgress = window.nestAI.onInstallProgress((progress) => {
      setProgressMap((current) => ({
        ...current,
    ...
    const unsubscribeOpenToolUi = window.nestAI.onOpenToolUi((payload) => {
      openEmbeddedToolUi(payload?.toolId);
    });

    const unsubscribeUpdateReady = window.nestAI.onUpdateReady((payload) => {
      pushToast(payload?.message || 'An update is ready. Restart NestAI to install.', 'success', {
        tag: 'update-ready',
        persistent: true,
        actionLabel: 'Restart Now',
        onAction: async () => {
          const result = await window.nestAI.restartToUpdate();
          if (!result?.ok) {
            pushToast(result?.message || 'NestAI could not restart to install the update.', 'error');
          }
        },
      });
    });
    ...
    const intervalId = window.setInterval(async () => {
      const result = await window.nestAI.refresh();
      if (result?.ok) {
        setAppState(result.data);
      }
    }, 5000);
  ...
  if (isLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-shell text-white">
        <div className="panel px-8 py-6 text-lg font-medium">Loading NestAI...</div>
      </div>
    );
  }
  ...
  if (appState.firstLaunch && appState.hardware) {
    return (
      <HardwareGate
        ...
        onContinue={() => runAction('first-launch', () => window.nestAI.completeFirstLaunch())}
      />
    );
  }
    ...
    <div className="min-h-screen bg-shell px-5 py-5 text-white lg:px-6">
      <div className="mx-auto grid max-w-[1600px] gap-5 xl:grid-cols-[280px,1fr]">
        <Sidebar
          ...
          onOpenLogs={() => runAction('open-logs', () => window.nestAI.openLogsFolder())}
            ...
            <section className="space-y-4">
              {ollamaTool && ollamaChatOpen ? (
                <OllamaChatPanel
                  ...
                  onLaunch={(toolId) => runAction(`launch:${toolId}`, () => window.nestAI.launchTool(toolId))}
                  onRefreshModels={() => loadOllamaModels()}
                  onSend={sendOllamaMessage}
                  onStop={(toolId) => runAction(`stop:${toolId}`, () => window.nestAI.stopTool(toolId))}
              ...
              {tools.length ? (
                tools.map((tool) => (
                  <LibraryCard
                    ...
                    busyMap={busyMap}
                    onLaunch={(toolId) => runAction(`launch:${toolId}`, () => window.nestAI.launchTool(toolId))}
                    onOpenFolder={(toolId) => runAction(`folder:${toolId}`, () => window.nestAI.openToolFolder(toolId))}
                    onOpenInterface={openEmbeddedToolUi}
                    onRepair={(toolId) => runAction(`repair:${toolId}`, () => window.nestAI.repairTool(toolId))}
                    onRestoreSnapshot={(toolId, snapshotFileName) =>
                      runAction(`restore:${toolId}`, () => window.nestAI.restoreSnapshot({ toolId, snapshotFileName }))
                    }
                    onSaveSnapshot={(toolId) => runAction(`snapshot:${toolId}`, () => window.nestAI.saveSnapshot(toolId))}
                    onStop={(toolId) => runAction(`stop:${toolId}`, () => window.nestAI.stopTool(toolId))}
                ...
                <div className="panel p-10 text-center">
                  ...
                  <p className="mx-auto mt-3 max-w-2xl text-sm leading-7 text-slate-300">
                    Switch to Store to install a supported local AI tool or let NestAI detect software already installed on this machine.
                  </p>
            ...
            <section className="panel p-6">
              <div className="flex flex-wrap items-end justify-between gap-4">
                <div>
                  ...
                  <h3 className="mt-3 text-3xl font-semibold text-white">Browse tools that NestAI can install for you</h3>
                </div>
                ...
                {storeTools.length ? (
                  storeTools.map((manifest) => (
                    <StoreCard
                      ...
                      manifest={manifest}
                      onInstall={(toolId) => runAction(`install:${toolId}`, () => window.nestAI.installTool(toolId))}
